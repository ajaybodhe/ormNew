...
func (this * NsqQueue) createPublisher() {
	...
	this.nsqProd, err = nsq.NewProducer(conf.OrmNewConfig.NSQ.Host + ":" + conf.OrmNewConfig.NSQ.Port, config)
...
func (this *NsqQueue) createConsumer() error{
	...
	this.nsqCons, err = nsq.NewConsumer(this.Topic, conf.OrmNewConfig.Consumer.ChannelName, config)
	if err != nil {
	...
	err = this.nsqCons.ConnectToNSQD(conf.OrmNewConfig.NSQ.Host +":" + conf.OrmNewConfig.NSQ.Port)
	if err != nil {
